...
const totalQuantity = cartItems.reduce((acc, item) => acc + item.quantity, null);
  ...
  const Slidebar = [
    { icon: <BoltIcon className="w-6 h-5 text-gray-600"/>, htext: "SuperCoin Zone" },
    { icon: <SparklesIcon className="w-6 h-5 text-gray-600" />, htext: "Flipkart Plus Zone" },
    { icon: <Squares2X2Icon className="w-6 h-5 text-gray-600"/>, htext: "All Categories" },
    { icon: <EllipsisVerticalIcon className="w-6 h-5 text-gray-600"/>, htext: "More on Flipkart" },
    { icon: <GlobeAltIcon className="w-6 h-5 text-gray-600" />, htext: "Choose Language" },
    { icon: <	TicketIcon className="w-6 h-5 text-gray-600"/>, htext: "Offer Zone" },
    { icon: <BriefcaseIcon className="w-6 h-5 text-gray-600"/>, htext: "Sell on Flipkart" },
    { icon: <ClipboardDocumentIcon className="w-6 h-5 text-gray-600"/>, htext: "My Orders" },
    {
      icon: <TicketIcon className="w-6 h-5 text-gray-600" />,
      htext: "Coupons",
    },
    {
      icon: <ShoppingCartIcon className="w-6 h-5 text-gray-600" />,
      htext: "My Cart",
    },
    {
      icon: <HeartIcon className="w-6 h-5 text-gray-600" />,
      htext: "My Wishlist",
    },
    { icon: <UserIcon className="w-6 h-5 text-gray-600"/>, htext: "My Account" },
    { icon: <BellIcon className="w-6 h-5 text-gray-600"/>, htext: "My Notification" },
  ...
  const HoveredItems = [
    {
      icon: <img src={Three_DotsN} alt="Notifications" className="size-5" />,
      htext: "Notification Preference",
    },
    {
      icon: <img src={Three_DotsN1} alt="Support" className="size-5" />,
      htext: "24x7 Customer Care",
    },
    {
      icon: <img src={Three_DotsN3} alt="Advertise" className="size-5" />,
      htext: "Advertise",
    },
    {
      icon: <img src={Three_DotsN2} alt="App" className="size-5" />,
      htext: "Download App",
    },
  ];
  const LoginHovered = [
  ...
  return (
    <div className="grid p-2 relative">
      ...
      <div className="flex md:justify-evenly justify-between items-center border md:border-slate-200 gap-3 w-full md:py-5">
        <Link to="/">
          <div className="flex justify-start items-center gap-0 w-full ">
            <button
              className="w-1/4 text-2xl md:hidden "
        ...
        <div className="w-2/5 border-2 border-blue-50 rounded-lg bg-blue-50 hidden md:flex">
          ...
          <input
            ...
            className="w-full pl-2 py-2 bg-blue-50"
          /> */}
          <SearchBar/>
        </div>
              ...
              <div className="flex items-center justify-center" key={index}>
                <Link to={item.path || "#"}>
                  <div
                    ...
                    ${
                      index === 1
                        ? "group hover:bg-blue-600 hover:text-white p-[8px] rounded-lg"
                        : ""
                    }
                    ${index >= 3 ? "hidden md:flex  " : ""}
                     ${
                       index === 4
                         ? "border border-slate-200 flex justify-center pl-1 bg-slate-100  "
                         : ""
                     }
                    `}
                    onMouseEnter={() => {
                  ...
                  ${index >= 3 ? "hidden md:block" : ""}
                   `}
                    >
                      {item.icon}{
                        index === 2 && totalQuantity > 0 && (
                          <span className="absolute -top-1 -right-[4px] bg-red-600 text-white text-xs px-[6px] py-[1px] rounded-full">{totalQuantity}</span>
                        )
                      }
                    </div>
                    <span className={`${index > 1 ? "hidden md:block" : ""}`}>
                      {index === 1 && isLogin ? "Logout" : item.htext}
                    </span>
                    {index === 1 && (
                ...
                {hovered1 === 1 && (
                  <div
                    className="absolute top-9 left-0 mt-2  bg-white border rounded shadow-md z-50  pt-2 pl-[5px] w-64 pr-3"
                  ...
                  >
                    <div>
                      <div className="flex p-2 justify-between mb-2 border-b-[3px] border-b-gray-100">
                        ...
                        <Link to="/login">
                          <button className="text-blue-700 text-[16px] font-semibold">
                            Sign Up
                          </button>
                        </Link>
                      </div>
                      {LoginHovered.map((item, index) => (
                        <div
                          ...
                          onClick={() =>
                            console.log(`Clicked on ${item.htext}`)
                          }
                        >
                          {item.icon}
                ...
                {hovered === 4 && (
                  <div
                    className="absolute top-7 right-0 mt-2 bg-white border rounded shadow-md z-50 p-2 pt-2 pl-[5px] w-60 pr-3"
                  ...
                  >
                    {HoveredItems.map((hoverItem, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 p-2 py-2 hover:bg-gray-100 rounded"
                      >
                        {hoverItem.icon}
      ...
      {open && (
        <div
          ref={slideRef}
          className="fixed top-0 left-0 h-full w-3/4 bg-white text-gray-600 shadow-lg z-20"
        >
            ...
            {Slidebar.map((item, index) => (
              <div
                ...
                className={`flex p-2 gap-2 items-center ${
                  [1, 3, 4, 6, 12].includes(index) ? "border-b" : ""
                }`}
              >
                <span>{item.icon}</span>
                <span>{item.htext}</span>
