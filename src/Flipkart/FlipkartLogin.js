...
const handleClickOutside = () => {
  ...
  return (
    <div className='grid gap-4 bg-slate-100'>
      ...
      <div className='hidden md:flex justify-evenly py-2 border-[2px] border-slate-200 bg-white'>
        {
          Navbar2.map((item,index)=> (
            <div key={index} className='flex gap-[5px] items-center group   '>
              ...
              <div className={`size-[5px] border-black border-r-[1px] border-b-[1px] rotate-45 mt-1 transition-all duration-100 ease-in-out 
                ${[7,8].includes(index) ?"hidden":""} group-hover:-rotate-[135deg] group-hover:border-r-blue-700 group-hover:border-b-blue-700`}></div>
            </div>
          ))
        }
      </div>
        ...
        <div className='flex w-full px-[17%] bg-white'>
          <div className='w-2/5 border  bg-[#257de1] hidden md:block'>
            <div className=' p-10 '>
              ...
              <p className='text-lg font-semibold text-slate-200 pt-5'>Get access to your Orders,<br></br>Wishlist and Recommendations</p>
            </div>
            <div className='flex justify-center items-center p-10 mt-20'>
              <img src={FlipkartImage1}></img>
            </div>
          </div>
